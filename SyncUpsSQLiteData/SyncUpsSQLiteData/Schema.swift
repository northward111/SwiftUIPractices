...
struct SyncUp: Hashable, Identifiable {
    ...
    var title = ""
    static let mock = SyncUp(id: UUID(), title: "Morning gathering")
    ...
    var duration: Duration {
        ...
        set {
            seconds = Int(newValue.components.seconds)
        }
    }
}

extension SyncUp.Draft: Hashable {
    var duration: Duration {
        get {
            .seconds(seconds)
        }
    ...
    func seedSampleData() throws {
        try seed {
            ...
            for name in [
                ...
                Attendee.Draft(syncUpID: UUID(1), name: name)
            }
            for name in ["Blob", "Blob Jr"] {
                Attendee.Draft(syncUpID: UUID(2), name: name)
            }
            for name in ["Blob Sr", "Blob Jr"] {
                Attendee.Draft(syncUpID: UUID(3), name: name)
            }
