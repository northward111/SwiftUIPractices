        ...
        Reduce {
            ...
            case .timerTick:
                ...
                let secondsPerAttendee = Int(
                    state.syncUp.durationPerAttendee.components.seconds
                )
...
//        appDatabase.saveSyncUp(state.syncUp)
    }
}
...
struct MeetingTimerView: View {
    ...
    var body: some View {
        Circle()
            ...
            .overlay {
                ForEach(
                    Array(syncUp.attendees.enumerated()),
                    id: \.element.id
                ) {
                    index,
                    attendee in
        ...
        VStack {
            HStack {
                if speakerIndex < syncUp.attendees.count - 1 {
                    Text(
                        "Speaker \(speakerIndex + 1) of \(syncUp.attendees.count)"
                    )
                } else {
