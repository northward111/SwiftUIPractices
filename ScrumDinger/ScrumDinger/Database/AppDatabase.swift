...
struct AppDatabase: Sendable {
    ...
    private var migrator: DatabaseMigrator {
        ...
        migrator.registerMigration("v1") { db in
            ...
            try SyncUp.createTable(db: db)
        }
        
        migrator.registerMigration("v2") { db in
            // Create a table
            // See <https://swiftpackageindex.com/groue/GRDB.swift/documentation/grdb/databaseschema>
            try Attendee.createTable(db: db)
            try Meeting.createTable(db: db)
        }
...
extension AppDatabase {
    func fetchAllSyncUps() -> [SyncUp] {
        do {
            return try reader.read { db in
                try SyncUp.fetchAllWithMeetingsAndAttendees(db)
            }
        } catch {
    ...
    func saveSyncUp(_ syncup: SyncUp) {
        do {
            try dbWriter.write { db in
                try syncup.saveSelfMeetingsAttendees(db: db)
            }
        } catch {
            print(error.localizedDescription)
        }
    ...
    func deleteSyncUps(ids: [UUID]) {
        do {
            try dbWriter.write { db in
                _ = try SyncUp.deleteAll(db, keys: ids)
            }
        } catch {
            print(error.localizedDescription)
        }
    ...
    @MainActor
    func observeSyncUps() -> AsyncThrowingStream<[SyncUp], Error> {
...
//    func refreshSyncUps() async throws {
//        try await dbWriter.write { db in
//            if try SyncUp.all().isEmpty(db) {
//                // When database is empty, insert new random players
//                try createRandomSyncUps(db)
//            } else {
//                // Insert a player
//                if Bool.random() {
//                    _ = try SyncUp.makeRandom().inserted(db) // insert but ignore inserted id
//                }
//                
...
//                
//                // Update some players
//                for var syncUp in try SyncUp.fetchAll(db) where Bool.random() {
//                    try syncUp.updateChanges(db) {
//                        $0.duration = .seconds(Int.random(in: 10..<100))
//                    }
//                }
//            }
//        }
//    }
    ...
    func createRandomSyncUpsIfEmpty() throws {
        try dbWriter.write { db in
            if try SyncUp.all().isEmpty(db) {
                try createRandomSyncUps(db)
            }
    ...
    private func createRandomSyncUps(_ db: Database) throws {
        for _ in 0..<3 {
            let randomSyncUp = SyncUp.makeRandom()
            try randomSyncUp.saveSelfMeetingsAttendees(db: db)
        }
    }
}
